...
void import() {
  cv::Mat img1_cvmat = cv::imread( "square3.bmp", 1 );
...
0.003663, 0.014652,  0.025641,  0.014652,  0.003663
      ...
      for (int x = 0; x < cols; x++) {
          ...
          for (int i=imgT; i<=imgB; i++){
              for (int j=imgL; j<=imgR; j++){
                  ...
                  //a= a+pow(grad_x.Pixel(j,i,0),2)*gaussian5x5[(gaus_ks-(y-i))*5+gaus_ks-(x-j)];
                //c= c+pow(grad_y.Pixel(j,i,0),2)*gaussian5x5[(gaus_ks-(y-i))*5+gaus_ks-(x-j)];
                //b= b+ grad_x.Pixel(j,i,0)*grad_y.Pixel(j,i,0)*gaussian5x5[(gaus_ks-(y-i))*5+gaus_ks-(x-j)];
              }
          }
  ...
  for (int y = 0; y < rows; y++) {
      for (int x = 0; x < cols; x++) {
          ...
          img1_ang << p;
          //printf("(%i, %i): %i \n", x, y, degImage[y*rows+x]);
          //printf("%i ", degImage[y*rows+x]);
      }
      //printf(";\n");
  }
...
void test_rotate(){
  ...
  rot_mat = getRotationMatrix2D(cv::Point(src.cols/2, src.rows/2), rotationAngle, scale);
  ...
  /*
  IplImage out_ipl = new_img_mat;
  //Mat2IplImage(warp_rotate_dst, out_ipl);
  cvSaveImage("input_rotated.bmp", &out_ipl);*/
  cv::imwrite("input_rotated2.bmp", new_img_mat);
